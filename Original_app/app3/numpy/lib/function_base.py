...
def rot90(m, k=1, axes=(0, 1)):
    ...
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.rot90', 'rot90(m, k=1, axes=(0, 1))', {'asanyarray': asanyarray, 'absolute': absolute, 'flip': flip, 'arange': arange, 'transpose': transpose, 'array_function_dispatch': array_function_dispatch, '_rot90_dispatcher': _rot90_dispatcher, 'm': m, 'k': k, 'axes': axes}, 1)
...
def average(a, axis=None, weights=None, returned=False, *, keepdims=np._NoValue):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.average', 'average(a, axis=None, weights=None, returned=False, keepdims=np._NoValue)', {'np': np, 'array_function_dispatch': array_function_dispatch, '_average_dispatcher': _average_dispatcher, 'a': a, 'axis': axis, 'weights': weights, 'returned': returned, 'keepdims': keepdims}, 2)
...
def asarray_chkfinite(a, dtype=None, order=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.asarray_chkfinite', 'asarray_chkfinite(a, dtype=None, order=None)', {'asarray': asarray, 'typecodes': typecodes, 'np': np, 'set_module': set_module, 'a': a, 'dtype': dtype, 'order': order}, 1)

def _piecewise_dispatcher(x, condlist, funclist, *args, **kw):
    import custom_funtemplate
    custom_funtemplate.rewrite_template('numpy.lib.function_base._piecewise_dispatcher', '_piecewise_dispatcher(x, condlist, funclist, *args, **kw)', {'np': np, 'x': x, 'condlist': condlist, 'funclist': funclist, 'args': args, 'kw': kw}, 0)
...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.piecewise', 'piecewise(x, condlist, funclist, *args, **kw)', {'asanyarray': asanyarray, 'isscalar': isscalar, 'ndarray': ndarray, 'asarray': asarray, 'np': np, 'zeros_like': zeros_like, 'collections': collections, 'array_function_dispatch': array_function_dispatch, '_piecewise_dispatcher': _piecewise_dispatcher, 'x': x, 'condlist': condlist, 'funclist': funclist, 'args': args, 'kw': kw}, 1)
...
def select(condlist, choicelist, default=0):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.select', 'select(condlist, choicelist, default=0)', {'np': np, 'array_function_dispatch': array_function_dispatch, '_select_dispatcher': _select_dispatcher, 'condlist': condlist, 'choicelist': choicelist, 'default': default}, 1)
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    .. math::
        ...
        \eta_{i} = f_{i}^{\left(1
ight)} -
                    \left[ lpha f\left(x_{i}
ight) +
                            eta f\left(x_{i} + h_{d}
ight) +
                            \gamma f\left(x_{i}-h_{s}
ight)
...
ight]
    ...
    .. math::
        ...
        \left\{
            egin{array}{r}
                lpha+eta+\gamma=0                 eta h_{d}-\gamma h_{s}=1                 eta h_{d}^{2}+\gamma h_{s}^{2}=0
            \end{array}
...
ight.
    ...
    .. math::
        ...
        \hat f_{i}^{(1)} =
            rac{
                h_{s}^{2}f\left(x_{i} + h_{d}
ight)
                + \left(h_{d}^{2} - h_{s}^{2}
ight)f\left(x_{i}
ight)
                - h_{d}^{2}f\left(x_{i}-h_{s}
ight)}
                { h_{s}h_{d}\left(h_{d} + h_{s}
ight)}
            + \mathcal{O}\left(rac{h_{d}h_{s}^{2}
                                + h_{s}h_{d}^{2}}{h_{d}
                                + h_{s}}
ight)
...
ight)
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.gradient', 'gradient(f, *varargs, axis=None, edge_order=1)', {'np': np, '_nx': _nx, 'array_function_dispatch': array_function_dispatch, '_gradient_dispatcher': _gradient_dispatcher, 'f': f, 'axis': axis, 'edge_order': edge_order, 'varargs': varargs}, 1)
...
def interp(x, xp, fp, left=None, right=None, period=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.interp', 'interp(x, xp, fp, left=None, right=None, period=None)', {'np': np, 'compiled_interp_complex': compiled_interp_complex, 'compiled_interp': compiled_interp, 'array_function_dispatch': array_function_dispatch, '_interp_dispatcher': _interp_dispatcher, 'x': x, 'xp': xp, 'fp': fp, 'left': left, 'right': right, 'period': period}, 1)
...
def unwrap(p, discont=None, axis=-1, *, period=2 * pi):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.unwrap', 'unwrap(p, discont=None, axis=-1, period=2 * pi)', {'asarray': asarray, 'diff': diff, 'np': np, '_nx': _nx, 'mod': mod, 'array': array, 'array_function_dispatch': array_function_dispatch, '_unwrap_dispatcher': _unwrap_dispatcher, 'p': p, 'discont': discont, 'axis': axis, 'period': period}, 1)
...
def sort_complex(a):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.sort_complex', 'sort_complex(a)', {'array': array, '_nx': _nx, 'array_function_dispatch': array_function_dispatch, '_sort_complex': _sort_complex, 'a': a}, 1)
...
def place(arr, mask, vals):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.place', 'place(arr, mask, vals)', {'np': np, '_insert': _insert, 'array_function_dispatch': array_function_dispatch, '_place_dispatcher': _place_dispatcher, 'arr': arr, 'mask': mask, 'vals': vals}, 1)
...
'
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.disp', 'disp(mesg, device=None, linefeed=True)', {'sys': sys, 'mesg': mesg, 'device': device, 'linefeed': linefeed}, 1)
_DIMENSION_NAME = '\\w+'
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None, aweights=None, *, dtype=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.cov', 'cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None, aweights=None, dtype=None)', {'np': np, 'array': array, 'average': average, 'warnings': warnings, 'dot': dot, 'array_function_dispatch': array_function_dispatch, '_cov_dispatcher': _cov_dispatcher, 'm': m, 'y': y, 'rowvar': rowvar, 'bias': bias, 'ddof': ddof, 'fweights': fweights, 'aweights': aweights, 'dtype': dtype}, 1)
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *, dtype=None):
    ...
    .. math:: R_{ij} = rac{ C_{ij} } { \sqrt{ C_{ii} C_{jj} } }
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.corrcoef', 'corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, dtype=None)', {'np': np, 'warnings': warnings, 'cov': cov, 'diag': diag, 'sqrt': sqrt, 'array_function_dispatch': array_function_dispatch, '_corrcoef_dispatcher': _corrcoef_dispatcher, 'x': x, 'y': y, 'rowvar': rowvar, 'bias': bias, 'ddof': ddof, 'dtype': dtype}, 1)
...
def blackman(M):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.blackman', 'blackman(M)', {'array': array, 'np': np, 'ones': ones, 'arange': arange, 'cos': cos, 'pi': pi, 'set_module': set_module, 'M': M}, 1)
...
def bartlett(M):
    ...
    .. math:: w(n) = rac{2}{M-1} \left(
              rac{M-1}{2} - \left|n - rac{M-1}{2}
ight|
...
ight)
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.bartlett', 'bartlett(M)', {'array': array, 'np': np, 'ones': ones, 'arange': arange, 'where': where, 'less_equal': less_equal, 'set_module': set_module, 'M': M}, 1)
...
def hanning(M):
    ...
    .. math::  w(n) = 0.5 - 0.5\cos\left(rac{2\pi{n}}{M-1}
ight)
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.hanning', 'hanning(M)', {'array': array, 'np': np, 'ones': ones, 'arange': arange, 'cos': cos, 'pi': pi, 'set_module': set_module, 'M': M}, 1)
...
def hamming(M):
    ...
    .. math::  w(n) = 0.54 - 0.46\cos\left(rac{2\pi{n}}{M-1}
ight)
               \qquad 0 \leq n \leq M-1
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.hamming', 'hamming(M)', {'array': array, 'np': np, 'ones': ones, 'arange': arange, 'cos': cos, 'pi': pi, 'set_module': set_module, 'M': M}, 1)
...
def _chbevl(x, vals):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base._chbevl', '_chbevl(x, vals)', {'x': x, 'vals': vals}, 1)
...
def i0(x):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.i0', 'i0(x)', {'np': np, 'piecewise': piecewise, '_i0_1': _i0_1, '_i0_2': _i0_2, 'array_function_dispatch': array_function_dispatch, '_i0_dispatcher': _i0_dispatcher, 'x': x}, 1)
...
def kaiser(M, beta):
    ...
    .. math::  w(n) = I_0\left( eta \sqrt{1-rac{4n^2}{(M-1)^2}}

ight)/I_0(eta)

    with

    .. math:: \quad -rac{M-1}{2} \leq n \leq rac{M-1}{2},
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.kaiser', 'kaiser(M, beta)', {'np': np, 'arange': arange, 'i0': i0, 'sqrt': sqrt, 'set_module': set_module, 'M': M, 'beta': beta}, 1)
...
e 0`. ``sinc(0)`` takes the limit value 1, making ``sinc`` not
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.sinc', 'sinc(x)', {'np': np, 'pi': pi, 'where': where, 'sin': sin, 'array_function_dispatch': array_function_dispatch, '_sinc_dispatcher': _sinc_dispatcher, 'x': x}, 1)
...
def msort(a):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.msort', 'msort(a)', {'warnings': warnings, 'array': array, 'array_function_dispatch': array_function_dispatch, '_msort_dispatcher': _msort_dispatcher, 'a': a}, 1)
...
def quantile(a, q, axis=None, out=None, overwrite_input=False, method='linear', keepdims=False, *, interpolation=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.quantile', "quantile(a, q, axis=None, out=None, overwrite_input=False, method='linear', keepdims=False, interpolation=None)", {'_check_interpolation_as_method': _check_interpolation_as_method, 'np': np, '_quantile_is_valid': _quantile_is_valid, '_quantile_unchecked': _quantile_unchecked, 'array_function_dispatch': array_function_dispatch, '_quantile_dispatcher': _quantile_dispatcher, 'a': a, 'q': q, 'axis': axis, 'out': out, 'overwrite_input': overwrite_input, 'method': method, 'keepdims': keepdims, 'interpolation': interpolation}, 1)
...
ight|_{x=x(t)} dt`.
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.trapz', 'trapz(y, x=None, dx=1.0, axis=-1)', {'asanyarray': asanyarray, 'diff': diff, 'np': np, 'array_function_dispatch': array_function_dispatch, '_trapz_dispatcher': _trapz_dispatcher, 'y': y, 'x': x, 'dx': dx, 'axis': axis}, 1)
...
def delete(arr, obj, axis=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.delete', 'delete(arr, obj, axis=None)', {'ndarray': ndarray, 'asarray': asarray, 'normalize_axis_index': normalize_axis_index, 'empty': empty, 'ones': ones, 'integer': integer, 'np': np, 'intp': intp, 'array_function_dispatch': array_function_dispatch, '_delete_dispatcher': _delete_dispatcher, 'arr': arr, 'obj': obj, 'axis': axis}, 1)
...
def insert(arr, obj, values, axis=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.insert', 'insert(arr, obj, values, axis=None)', {'ndarray': ndarray, 'asarray': asarray, 'normalize_axis_index': normalize_axis_index, 'arange': arange, 'intp': intp, 'np': np, 'warnings': warnings, 'array': array, 'empty': empty, 'ones': ones, 'array_function_dispatch': array_function_dispatch, '_insert_dispatcher': _insert_dispatcher, 'arr': arr, 'obj': obj, 'values': values, 'axis': axis}, 1)
...
def digitize(x, bins, right=False):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.lib.function_base.digitize', 'digitize(x, bins, right=False)', {'_nx': _nx, 'np': np, '_monotonicity': _monotonicity, 'array_function_dispatch': array_function_dispatch, '_digitize_dispatcher': _digitize_dispatcher, 'x': x, 'bins': bins, 'right': right}, 1)
