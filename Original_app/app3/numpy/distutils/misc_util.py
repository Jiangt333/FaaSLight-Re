        ...
        if (os.path.isfile(target) or revision is None):
            ...
            def generate_svn_version_py():
                ...
                def rm_file(f=target, p=self.info):
                    import custom_funtemplate
                    custom_funtemplate.rewrite_template('numpy.distutils.misc_util.Configuration.make_svn_version_py.generate_svn_version_py.rm_file', 'rm_file(f=target, p=self.info)', {'delete': delete, 'os': os, 'f': f, 'p': p, 'target': target}, 0)
    ...
    def make_hg_version_py(self, delete=True):
        ...
        if (os.path.isfile(target) or revision is None):
            ...
            def generate_hg_version_py():
                ...
                def rm_file(f=target, p=self.info):
                    import custom_funtemplate
                    custom_funtemplate.rewrite_template('numpy.distutils.misc_util.Configuration.make_hg_version_py.generate_hg_version_py.rm_file', 'rm_file(f=target, p=self.info)', {'delete': delete, 'os': os, 'f': f, 'p': p, 'target': target}, 0)
                atexit.register(rm_file)
                return target
