...
def findf90modules(m):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=findf90modules=34')
    if ismodule(m):
        return [m]
    if not hasbody(m):
        return []
...
def buildhooks(pymod):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=buildhooks=87')
    from . import rules
    ret = {'f90modhooks': [], 'initf90modhooks': [], 'body': [], 'need': ['F_FUNC', 'arrayobject.h'], 'separatorsfor': {'includes0': '\n', 'includes': '\n'}, 'docs': ['"Fortran 90/95 modules:\\n"'], 'latexdoc': []}
    fhooks = ['']
    
    def fadd(line, s=fhooks):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=buildhooks=fadd=96')
        s[0] = '%s\n      %s' % (s[0], line)
    doc = ['']
    
    def dadd(line, s=doc):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=buildhooks=dadd=100')
        s[0] = '%s\n%s' % (s[0], line)
    for m in findf90modules(pymod):
        ...
        def cadd(line, s=chooks):
            print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=buildhooks=cadd=122')
            s[0] = '%s\n%s' % (s[0], line)
        ihooks = ['']
        
        def iadd(line, s=ihooks):
            print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/f2py/f90mod_rules.py=buildhooks=iadd=126')
            s[0] = '%s\n%s' % (s[0], line)
