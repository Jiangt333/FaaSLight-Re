...
class CallOnceOnly:
    ...
    def __init__(self):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=CallOnceOnly=__init__=48')
        self._check_types = None
        self._check_ieee_macros = None
        self._check_complex = None
    
    def check_types(self, *a, **kw):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=CallOnceOnly=check_types=53')
    ...
    def check_ieee_macros(self, *a, **kw):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=CallOnceOnly=check_ieee_macros=61')
    ...
    def check_complex(self, *a, **kw):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=CallOnceOnly=check_complex=69')
...
def can_link_svml():
    ...
    only on linux
    """
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=can_link_svml=77')
    if NPY_DISABLE_SVML:
        return False
...
def check_svml_submodule(svmlpath):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_svml_submodule=88')
...
def pythonlib_dir():
    ...
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=pythonlib_dir=94')
...
def is_npy_no_signal():
    ...
    header."""
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=is_npy_no_signal=101')
    return sys.platform == 'win32'

def is_npy_no_smp():
    """Return True if the NPY_NO_SMP symbol must be defined in public
    header (when SMP support cannot be reliably enabled)."""
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=is_npy_no_smp=106')
    return 'NPY_NOSMP' in os.environ

def win32_checks(deflist):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=win32_checks=117')
...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_math_capabilities=133')
    
    def check_func(func_name, decl=False, headers=['feature_detection_math.h', 'feature_detection_cmath.h']):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_math_capabilities=check_func=134')
        return config.check_func(func_name, libraries=mathlibs, decl=decl, call=True, call_args=FUNC_CALL_ARGS[func_name], headers=headers)
    
    def check_funcs_once(funcs_name, headers=['feature_detection_math.h', 'feature_detection_cmath.h'], add_to_moredefs=True):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_math_capabilities=check_funcs_once=148')
        call = dict([(f, True) for f in funcs_name])
        call_args = dict([(f, FUNC_CALL_ARGS[f]) for f in funcs_name])
        st = config.check_funcs_once(funcs_name, libraries=mathlibs, decl=False, call=call, call_args=call_args, headers=headers)
        if (st and add_to_moredefs):
            moredefs.extend([(fname2def(f), 1) for f in funcs_name])
        return st
    
    def check_funcs(funcs_name, headers=['feature_detection_math.h', 'feature_detection_cmath.h']):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_math_capabilities=check_funcs=166')
    ...
    def check_gh14787(fn):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_math_capabilities=check_gh14787=182')
        if fn == 'attribute_target_avx512f':
            if (sys.platform in ('win32', 'cygwin') and config.check_compiler_gcc() and not config.check_gcc_version_at_least(8, 4)):
    ...
    if not check_funcs_once(MANDATORY_FUNCS, add_to_moredefs=False):
        raise SystemError('One of the required function to build numpy is not available (the list is %s).' % str(MANDATORY_FUNCS))
...
def check_complex(config, mathlibs):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_complex=259')
    priv = []
    pub = []
    ...
    if st:
        ...
        def check_prec(prec):
            print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_complex=check_prec=274')
...
def check_ieee_macros(config):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_ieee_macros=292')
    priv = []
    pub = []
    macros = []
    
    def _add_decl(f):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_ieee_macros=_add_decl=298')
...
def check_types(config_cmd, ext, build_dir):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_types=329')
    private_defines = []
    public_defines = []
    expected = {'short': [2], 'int': [4], 'long': [8, 4], 'float': [4], 'double': [8], 'long double': [16, 12, 8], 'Py_intptr_t': [8, 4], 'PY_LONG_LONG': [8], 'long long': [8], 'off_t': [8, 4]}
    ...
    for type in ('Py_intptr_t', 'off_t'):
        res = config_cmd.check_type_size(type, headers=['Python.h'], library_dirs=[pythonlib_dir()], expected=expected[type])
        if res >= 0:
            private_defines.append(('SIZEOF_%s' % sym2def(type), '%d' % res))
            public_defines.append(('NPY_SIZEOF_%s' % sym2def(type), '%d' % res))
        else:
    ...
    if config_cmd.check_decl('PY_LONG_LONG', headers=['Python.h']):
        res = config_cmd.check_type_size('PY_LONG_LONG', headers=['Python.h'], library_dirs=[pythonlib_dir()], expected=expected['PY_LONG_LONG'])
        if res >= 0:
...
def check_mathlib(config_cmd):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=check_mathlib=430')
    mathlibs = []
...
def visibility_define(config):
    ...
    string)."""
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=visibility_define=455')
...
def configuration(parent_package='', top_path=None):
    print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=464')
    ...
    def generate_config_h(ext, build_dir):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=generate_config_h=489')
    ...
    def generate_numpyconfig_h(ext, build_dir):
        ...
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=generate_numpyconfig_h=599')
    ...
    def generate_api_func(module_name):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=generate_api_func=667')
        
        def generate_api(ext, build_dir):
            print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=generate_api_func=generate_api=668')
    ...
    def get_mathlib_info(*args):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=get_mathlib_info=718')
        config_cmd = config.get_config_cmd()
        for (lang, test_code, note) in (('c', 'int main(void) { return 0;}', ''), ('c++', 'int main(void){ auto x = 0.0; return static_cast<int>(x); }', 'note: A compiler with support for C++11 language features is required.')):
            is_cpp = lang == 'c++'
            if is_cpp:
            ...
            if not st:
                ...
                raise RuntimeError(f'Broken toolchain: cannot link a simple {lang.upper()} program. {note}')
            if is_cpp:
    ...
    config.add_installed_library('npymath', sources=npymath_sources + [get_mathlib_info], install_dir='lib', build_info={'include_dirs': [], 'extra_compiler_args': [lib_opts_if_msvc]})
    ...
    config.add_extension('_multiarray_tests', sources=[join('src', 'multiarray', '_multiarray_tests.c.src'), join('src', 'common', 'mem_overlap.c'), join('src', 'common', 'npy_argparse.c'), join('src', 'common', 'npy_hashtable.c')], depends=[join('src', 'common', 'mem_overlap.h'), join('src', 'common', 'npy_argparse.h'), join('src', 'common', 'npy_hashtable.h'), join('src', 'common', 'npy_extint128.h')], libraries=['npymath'])
    common_deps = [join('src', 'common', 'dlpack', 'dlpack.h'), join('src', 'common', 'array_assign.h'), join('src', 'common', 'binop_override.h'), join('src', 'common', 'cblasfuncs.h'), join('src', 'common', 'lowlevel_strided_loops.h'), join('src', 'common', 'mem_overlap.h'), join('src', 'common', 'npy_argparse.h'), join('src', 'common', 'npy_cblas.h'), join('src', 'common', 'npy_config.h'), join('src', 'common', 'npy_ctypes.h'), join('src', 'common', 'npy_dlpack.h'), join('src', 'common', 'npy_extint128.h'), join('src', 'common', 'npy_import.h'), join('src', 'common', 'npy_hashtable.h'), join('src', 'common', 'npy_longdouble.h'), join('src', 'common', 'npy_svml.h'), join('src', 'common', 'templ_common.h.src'), join('src', 'common', 'ucsnarrow.h'), join('src', 'common', 'ufunc_override.h'), join('src', 'common', 'umathmodule.h'), join('src', 'common', 'numpyos.h'), join('src', 'common', 'npy_cpu_dispatch.h'), join('src', 'common', 'simd', 'simd.h')]
    common_src = [join('src', 'common', 'array_assign.c'), join('src', 'common', 'mem_overlap.c'), join('src', 'common', 'npy_argparse.c'), join('src', 'common', 'npy_hashtable.c'), join('src', 'common', 'npy_longdouble.c'), join('src', 'common', 'templ_common.h.src'), join('src', 'common', 'ucsnarrow.c'), join('src', 'common', 'ufunc_override.c'), join('src', 'common', 'numpyos.c'), join('src', 'common', 'npy_cpu_features.c')]
    ...
    def generate_umath_doc_header(ext, build_dir):
        print('/home/jiangt/FaaSLight-Re/Original_app/app3-dynamic/numpy/core/setup.py=configuration=generate_umath_doc_header=1027')
    ...
    umath_src = [join('src', 'umath', 'umathmodule.c'), join('src', 'umath', 'reduction.c'), join('src', 'umath', 'funcs.inc.src'), join('src', 'umath', 'simd.inc.src'), join('src', 'umath', 'loops.h.src'), join('src', 'umath', 'loops_utils.h.src'), join('src', 'umath', 'loops.c.src'), join('src', 'umath', 'loops_unary_fp.dispatch.c.src'), join('src', 'umath', 'loops_arithm_fp.dispatch.c.src'), join('src', 'umath', 'loops_arithmetic.dispatch.c.src'), join('src', 'umath', 'loops_minmax.dispatch.c.src'), join('src', 'umath', 'loops_trigonometric.dispatch.c.src'), join('src', 'umath', 'loops_umath_fp.dispatch.c.src'), join('src', 'umath', 'loops_exponent_log.dispatch.c.src'), join('src', 'umath', 'loops_hyperbolic.dispatch.c.src'), join('src', 'umath', 'loops_modulo.dispatch.c.src'), join('src', 'umath', 'loops_comparison.dispatch.c.src'), join('src', 'umath', 'matmul.h.src'), join('src', 'umath', 'matmul.c.src'), join('src', 'umath', 'clip.h'), join('src', 'umath', 'clip.cpp'), join('src', 'umath', 'dispatching.c'), join('src', 'umath', 'legacy_array_method.c'), join('src', 'umath', 'wrapping_array_method.c'), join('src', 'umath', 'ufunc_object.c'), join('src', 'umath', 'extobj.c'), join('src', 'umath', 'scalarmath.c.src'), join('src', 'umath', 'ufunc_type_resolution.c'), join('src', 'umath', 'override.c'), join('src', 'umath', 'string_ufuncs.cpp'), join('src', 'umath', '_scaled_float_dtype.c')]
    umath_deps = [generate_umath_py, join('include', 'numpy', 'npy_math.h'), join('include', 'numpy', 'halffloat.h'), join('src', 'multiarray', 'common.h'), join('src', 'multiarray', 'number.h'), join('src', 'common', 'templ_common.h.src'), join('src', 'umath', 'simd.inc.src'), join('src', 'umath', 'override.h'), join(codegen_dir, 'generate_ufunc_api.py'), join(codegen_dir, 'ufunc_docstrings.py')]
    ...
    config.add_extension('_simd', sources=[join('src', 'common', 'npy_cpu_features.c'), join('src', '_simd', '_simd.c'), join('src', '_simd', '_simd_inc.h.src'), join('src', '_simd', '_simd_data.inc.src'), join('src', '_simd', '_simd.dispatch.c.src')], depends=[join('src', 'common', 'npy_cpu_dispatch.h'), join('src', 'common', 'simd', 'simd.h'), join('src', '_simd', '_simd.h'), join('src', '_simd', '_simd_inc.h.src'), join('src', '_simd', '_simd_data.inc.src'), join('src', '_simd', '_simd_arg.inc'), join('src', '_simd', '_simd_convert.inc'), join('src', '_simd', '_simd_easyintrin.inc'), join('src', '_simd', '_simd_vector.inc')], libraries=['npymath'])
