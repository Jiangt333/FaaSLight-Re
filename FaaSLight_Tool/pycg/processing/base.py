...
# 这个bese类继承了ast，当使用analyze的时候，就会自动执行相应node类型的visitor方法。
class ProcessingBase(ast.NodeVisitor):
    def __init__(self, filename, modname, modules_analyzed):
        ...
        # print('read files========={}'.format(filename))
        ...
        # print('read file as-----{}'.format(filename))
        # self.filename = os.path.abspath(filename)

        # if filename.endswith('.so'):
        #     print('cannot read file -- so file as-----{}'.format(filename))
        #     # open('temp.txt', 'a').write(filename)
        ...
        def do_assign(decoded, target):
            ...
            if isinstance(target, ast.Tuple):
                for pos, elt in enumerate(target.elts):
                    if callable(hasattr(decoded, 'len')) and pos < len(decoded):
